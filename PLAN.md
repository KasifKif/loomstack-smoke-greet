...
required: false
acceptance:
  - file exists: this_path_does_not_exist_and_never_will.py
notes: |
  ...
  approval gate then blocks the task. Acceptance is intentionally
  unsatisfiable so a non-pre-seeded run would also reach attempt 3
  eventually — but the pre-seed makes this deterministic + cheap.
